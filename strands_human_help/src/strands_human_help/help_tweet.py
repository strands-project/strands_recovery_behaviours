...
class HelpTweet(UIHelper):
    ...
    def __init__(self, bumper_fails_to_tweet=20, nav_fails_to_tweet=20):
        ...
        self.got_server=self.speaker.wait_for_server(rospy.Duration(1))
        countdown=10
        while not self.got_server and countdown>0:
            rospy.loginfo("Help via twitter is waiting for grab image then tweet action..." + countdown)
            self.got_server=self.speaker.wait_for_server(rospy.Duration(1))
        ...
        self.magnetic_post="I'm very close to a flight of stairs! I'm frozen in terror! Please one of my handlers come and help me!"
        self.tweet=GrabImageThenTweetGoal(topic='head_xtion/rgb/image_mono')
